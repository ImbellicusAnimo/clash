...
- **Auth:** Session-basierte Authentifizierung mit **Username/Passwort (Credentials)** — kein OAuth/Social-Login. E-Mail bleibt als optionales Profil-Feld auf `User` bestehen (nicht im Registrierungsformular abgefragt, keine Registrierungspflicht, keine Verifizierungs-/Bestätigungs-Mail). Passwörter gehasht (bcrypt) im `User`-Modell speichern; signierte Session via `jose`-Cookie (`SESSION_SECRET` in `.env`).
